...
{if $formEdit}
  ...
  {if $cd_edit.is_multiple and ( ( $cd_edit.max_multiple eq '' )  or ( $cd_edit.max_multiple > 0 and $cd_edit.max_multiple > $cgCount ) ) }
    <div id="add-more-link-{$cgCount}" class="add-more-link-{$group_id} add-more-link-{$group_id}-{$cgCount}">
      <a href="#" class="crm-hover-button" onclick="CRM.buildCustomData('{$cd_edit.extends}',{if $cd_edit.subtype}'{$cd_edit.subtype}'{else}'{$cd_edit.extends_entity_column_id}'{/if}, '', {$cgCount}, {$group_id}, true, '', {$profileID} ); return false;">
...
{else}
  {foreach from=$groupTree item=cd_edit key=group_id name=custom_sets}
    {if ( $cd_edit.is_multiple and $multiRecordDisplay eq 'single' ) or $cd_edit.is_multiple eq 0 }
      <div class="custom-group custom-group-{$cd_edit.name}">
        {if !$skipTitle and $cgCount  eq 1}
     ...
     <div class="custom-group custom-group-{$cd_edit.name} crm-accordion-wrapper crm-custom-accordion {if $cd_edit.collapse_display and !$skipTitle}collapsed{/if}">
      ...
      <div class="crm-accordion-body">
        ...
        <table class="form-layout-compressed">
          ...
          <td>
          {if $cd_edit.is_multiple eq 1 and $cd_edit.table_id and !$skipTitle and $cd_edit.style eq 'Inline'}
          {assign var=tableID value=$cd_edit.table_id}
          
         <a href="#" id="crm-contrib-custom-value-del_{$tableID}" class="crm-hover-button crm-custom-value-del" title="{ts 1=$cd_edit.title}Delete %1{/ts}"
          data-post='{ldelim}"valueID": "{$tableID}", "groupID": "{$group_id}", "key": "{crmKey name='civicrm/ajax/multicustomvalue'}"{rdelim}'>
